...
<p align="justify"> MitoPhy is a visual interface written in python 3 for Unix computers dedicated to sequence management and molecular phylogenetics. The purpose of MitoPhy is to provide a quick and efficient way to build phylogenies de novo. We designed the general idea for MitoPhy during my Master's internship in 2022. I was asked to build a large phylogeny using mainly mitochondrial genes for more than 350 species. No sequences were already available in my lab, so I relied on using NCBI's GenBank. However, the task was a bit too overwhelming for me and before this internship, I had no genuine idea of how GenBank works. I asked myself several questions such as: what is a "good" sequence: is it a long one, a well-attributed one, or a contamination-free one? To make it simple, all of the above. But since my time was limited, I could not simply examine each possible gene for each species more than 5200 times and look for these criteria. Moreover, with such a large number, human errors can happen easily. So I decided to build a pipeline with several programs and scripts, making this task easier. </p>

<p align="justify"> Mitophy mainly relies on taking actions on directory recursively, meaning that when proposed to select a directory, the user should create a new one. Otherwise the built-in loop in the programm can go out of hand, and cause several issues. If any doubts, work only with pristine clear directories or kill the process in the terminal. MitoPhy is meant to be used thoroughly, as such  I recommend using commands on files/directories generated by the programm itself.</p>

<p align="justify"> Many programs used in MitoPhy were not developed by me such as muscle (Edgar, 2004), IQ-TREE (Minh et al., 2020) as well as python pacakges : module Bio (Cock et al., 2009), pandas (McKinney, W. et al., 2010), so please cite each of them when using the associated function(s). If you have any questions/requests feel free to ask me at my Github. </p>

## 2 Installation

MitoPhy is a standalone program with no prerequisites needed.

### 2.1 MitoPhy for Mac

MitoPhy.app is a bundled app that can be used like any other app from the App Store. However, this distribution does not display the terminal and bugs are hardly tracked down. To counteract this, the user can enter manually inside the MitoPhy.app and locate the MitoPhy executable file (MitoPhy/Contents/MacOS/MitoPhy). This procedure is quite handy as it can help you keep tracks along the procedure, such as phylogenetic reconstruction.
...
<p align="justify"> This panel allows the user to enter a .txt file as input. This file can have more than one species, each written on a different line.  </p>
...
This option allows the user to search for RefSeq sequences only. Refseq sequences are of high quality but are less represented than any non-RefSeq sequence, usually, each RefSeq sequence contains the whole mitogenome.

#### 3.3.2 Date range

<p align="justify"> This option allows the user to enter a date interval for the downloaded sequence(s). As such if one writes 2014/08/03 and 2022/03/01, the downloaded sequence(s) cannot be older than the third of august 2014 but are only younger than the first of march 2022. The format must be YYYY/MM/DD and both of the panels must be completed in a valid format. This step is optional, keep the panel blank if you do not want any date constraints. </p>

#### 3.3.3 Sequence range

<p align="justify"> This option allows the user to enter a sequence length interval in which the downloaded sequence cannot be smaller or bigger. Same as "Date range”, you must complete both of the panels if you want a constraint. This step is optional, keep the panel blank if you do not want any sequence length constraints. </p>

### 3.4 Research bar

<p align="justify"> At the moment, the research bar submenu only contains the research button. Taking into account all previous options and input, the research button will perform research on NCBI's GenBank database. For each species provided, a folder will be created along with a .gb (GenBank format) containing many sequences for the species (capped at 5000 to not overload the NCBI's servers). </p>
...
This panel allows the user to enter an input directory for the following operations. The structure of this directory includes two levels: the first is the species level, and the second is inside the first one. If nothing is written here, the previous from section [3.2.1](#321-Save-in-directory) will be entered.
...
• You have more than **two** sequences and a majority of the sequenced is made and moved to the output directory. Ambiguous sites are "N”.
...
With the directory selected in [4.1.1](#411-Select-the-operating-directory), aligns all the files in each subdirectory with muscle and creates output in the main root directory. All files must be in fasta format.
...
Merges all the .fasta files from the main directory by species names. Also creates two partition files in .txt format, which could be used in the [last chapter](#5-Phylogeny). All files must be in fasta formats (end with .faa) and sequences in each multi-sequence files must be of the same length.

#### 4.2.4 Alignement info

Select a multi-sequence file and give information about : the number of sequences, the number of invariable sites, variable sites and only indel sites. 
...
- Mandatory step
```
Selects an input multi-sequence of valid format to build the phylogeny. 
...
- Mandatory step
```
Selects an outgroup among the sequence referenced in your dataset.

#### 5.1.3 Select bootstrap value

Selects a non-parametric bootstrap value. 

#### 5.1.4 Select a constraint tree

Selects a tree in newick format to constraint some nodes in the phylogenetic analysis.
...
All options used for distance-based and maximum parsimony methods.
...
Select the best-suited algorithm for your data.
...
• **Parsimony** (Cladistic,  optimality criterion under which the phylogenetic tree minimizing the total number of character-state changes)
...
• **Majority consensus** methods only show relationships that are unambiguously supported by the data in the majority of the trees
...
This panel allows the user to build a distance-based or maximum parsimony tree using the options in sections [5.1](#51-General-options) and [5.2](#52-Distance-and-parsimony).
...
Selects a substitution model for your sequence, this option is overwritten when using partition files.
...
Uses discrete Gamma model for rate heterogeneity accross sites with 4 categories (Yang, 1994)

#### 5.3.4 Select the partition file

Selects a partition file for the analysis. Partitions are written in order to fit multiple substitution models for each of them. Partition files can be written by hand, but are also autogenerated by [step 4.2.3](#423-Merge) and ends with .nex.
...
• **Bootstrap** Non-parametric bootstrap is a random sampling with replacement creating pseudo-matrices for building alternative trees to be compared to the original one. Nodes with >= 70 % support are robust. Final tree ends with *.contree*.
...
- Mandatory step
```
Builds a maximum likelihood tree with IQ-TREE, several other files will also be created.
